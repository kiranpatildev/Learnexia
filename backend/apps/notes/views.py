...
class LectureNoteViewSet(viewsets.ModelViewSet):
    ...
    def is_bookmarked(self, request, pk=None):
        """
        Check if note is bookmarked by current student
        """
        note = self.get_object()
        is_bookmarked = StudentNoteBookmark.objects.filter(
            note=note,
            student=request.user
        ).exists()
    ...
    def generate_pdf(self, request, pk=None):
        """
        Generate and download PDF from note
        ...
        # Check permissions
        if request.user.role == 'teacher' and note.teacher != request.user:
            return Response(
                {'error': 'You can only generate PDF for your own notes'},
                status=status.HTTP_403_FORBIDDEN
            )
        
        if request.user.role == 'student' and not note.is_published:
            return Response(
                {'error': 'This note is not published yet'},
                status=status.HTTP_403_FORBIDDEN
            )
        ...
        try:
            ...
            # Add metadata
            metadata = f"""
            <b>Subject:</b> {note.classroom.subject.name if note.classroom and note.classroom.subject else 'N/A'}<br/>
            <b>Teacher:</b> {note.teacher.get_full_name()}<br/>
            <b>Date:</b> {note.created_at.strftime('%B %d, %Y')}<br/>
            """
            if note.is_auto_generated:
                metadata += "<b>Generated by:</b> AI<br/>"
            ...
            if note.summary:
                ...
                story.append(Paragraph(note.summary, body_style))
                story.append(Spacer(1, 0.2*inch))
